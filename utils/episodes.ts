...
export const isEpisodeStoryboardComplete = (ep?: Episode) => {
  if (!ep || ep.shots.length === 0) return false;
  return (
    ep.status === "review_storyboard" ||
    ep.shots.every((s) => s.storyboardPrompt && s.storyboardPrompt.trim().length > 0)
  );
};

export const findNextStoryboardIndex = (episodes: Episode[], startIndex = 0) => {
  for (let i = startIndex; i < episodes.length; i++) {
    const ep = episodes[i];
    if (!ep || ep.shots.length === 0) continue;
    if (!isEpisodeStoryboardComplete(ep)) return i;
  }
  return -1;
};

...
export const AppShell: React.FC<Props> = ({ header, sidebar, children, isDarkMode }) => {
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''} h-screen flex flex-col`}>
      <div className="flex flex-col h-screen bg-[var(--bg-panel)] text-[var(--text-primary)] transition-colors duration-300">
        {header}
        <main className="flex-1 flex overflow-hidden relative">
          ...
          {hasSidebar && sidebar}
          <section className="flex-1 overflow-hidden relative bg-[var(--bg-panel)] scroll-pt-24 md:scroll-pt-28">
            {children}
          </section>
        </main>
      </div>
